#!/bin/sh
# Day 9 PostgreSQL Database Connection and Seed Integrity Verifier
...
if pg_isready -h localhost -p 5432 -U postgres; then
    ...
    # Query row counts
    count=$(psql -h localhost -U postgres -d postgres -t -c "SELECT COUNT(*) FROM compose_tasks;" | tr -d '[:space:]')
